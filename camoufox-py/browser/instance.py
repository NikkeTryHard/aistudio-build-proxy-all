...
def run_browser_instance(config):
    ...
    logger = setup_logging(
        os.path.join("logs", "app.log"), prefix=f"{cookie_file_config}"
    )
    ...
    try:
        with open(cookie_file, "r") as f:
            ...
            # 支持两种格式：直接的cookie数组 或 包含"cookies"键的对象
            if isinstance(cookie_data, dict) and "cookies" in cookie_data:
                cookies_from_file = cookie_data["cookies"]
            elif isinstance(cookie_data, list):
                cookies_from_file = cookie_data
            else:
                logger.error(
                    f"Cookie 文件格式不正确，应该是一个数组或包含'cookies'键的对象"
                )
                return
    ...
    try:
        with Camoufox(**launch_options) as browser:
            ...
            try:
                ...
                response = page.goto(
                    expected_url, wait_until="domcontentloaded", timeout=120000
                )

                # 检查HTTP响应状态码
                if response:
                    logger.info(
                        f"导航初步成功，服务器响应状态码: {response.status} {response.status_text}"
                    )
                    if not response.ok:  # response.ok 检查状态码是否在 200-299 范围内
                        logger.warning(
                            f"警告：页面加载成功，但HTTP状态码表示错误: {response.status}"
                        )
                        # 即使状态码错误，也保存快照以供分析
                        page.screenshot(
                            path=os.path.join(
                                screenshot_dir,
                                f"WARN_http_status_{response.status}_{cookie_file_config}.png",
                            )
                        )
                else:
                ...
                logger.error(
                    "可能原因：网络连接缓慢、目标网站服务器无响应、代理问题、或页面资源被阻塞。"
                )
                # 尝试保存诊断信息
                try:
                    # 截图对于看到页面卡在什么状态非常有帮助（例如，空白页、加载中、Chrome错误页）
                    screenshot_path = os.path.join(
                        screenshot_dir, f"FAIL_timeout_{cookie_file_config}.png"
                    )
                    ...
                    html_path = os.path.join(
                        screenshot_dir, f"FAIL_timeout_{cookie_file_config}.html"
                    )
                    ...
                    logger.error(
                        f"在尝试进行超时诊断（截图/保存HTML）时发生额外错误: {diag_e}"
                    )
                ...
                try:
                    screenshot_path = os.path.join(
                        screenshot_dir, f"FAIL_network_error_{cookie_file_config}.png"
                    )
                    page.screenshot(path=screenshot_path)
                    logger.info(f"已截取网络错误时的屏幕快照: {screenshot_path}")
                except Exception as diag_e:
                    logger.error(
                        f"在尝试进行网络错误诊断（截图）时发生额外错误: {diag_e}"
                    )
                return  # 网络错误，终止
            ...
            if "accounts.google.com/v3/signin/identifier" in final_url:
                ...
                page.screenshot(
                    path=os.path.join(
                        screenshot_dir, f"FAIL_identifier_page_{cookie_file_config}.png"
                    )
                )
                return
                ...
                try:
                    ...
                    page.screenshot(
                        path=os.path.join(
                            screenshot_dir,
                            f"FAIL_spinner_stuck_{cookie_file_config}.png",
                        )
                    )
                ...
                if auth_error_locator.is_visible(timeout=2000):
                    logger.error(
                        f"检测到认证失败的错误横幅: '{auth_error_text}'. Cookie已过期或无效。"
                    )
                    screenshot_path = os.path.join(
                        screenshot_dir,
                        f"FAIL_auth_error_banner_{cookie_file_config}.png",
                    )
                    page.screenshot(path=screenshot_path)
                ...
                if login_button_cn.is_visible(
                    timeout=1000
                ) or login_button_en.is_visible(timeout=1000):
                    logger.error("页面上仍显示'登录'按钮。Cookie无效。")
                    page.screenshot(
                        path=os.path.join(
                            screenshot_dir,
                            f"FAIL_login_button_visible_{cookie_file_config}.png",
                        )
                    )
                    return
                ...
                page.screenshot(
                    path=os.path.join(
                        screenshot_dir,
                        f"FAIL_chooser_click_failed_{cookie_file_config}.png",
                    )
                )
                return
            else:
                ...
                page.screenshot(
                    path=os.path.join(
                        screenshot_dir, f"FAIL_unexpected_url_{cookie_file_config}.png"
                    )
                )
                return
